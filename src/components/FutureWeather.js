			...
			{data.map((day, i) => {
				return (
					<li className='card card--future js-card' key={i} data-card={i}>
						<button onClick={() => toggleOpen(i)} className='card__item-day' data-btn={i}>Day: {day[0].dt_txt.split(' ')[0]}</button>
...
export default function FutureWeather() {
	const [state, dispatch] = React.useReducer(
		...
		{data: null, error: null, loading: true, input: '', place: 'Minsk'}
	);
	...
	return (
		<div>
			...
			<input
				...
				className='input'
			/>
			<button className='btn' disabled={!input} onClick={() => dispatch({type: 'place', place: input})}>Search
			</button>
			{loading && <Loading/>}
